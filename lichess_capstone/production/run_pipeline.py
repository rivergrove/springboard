...
class Pipeline:
    ...
    def set_month(self, month):
        self.month = month
        self.folder = f'{self.root_folder}/{self.month}'
    ...
    def download_data(self):
        ...
        write_file = f'{self.folder}/{self.month}.pgn.bz2'
        open(write_file, 'wb').write(file.content)
        
        self.print_log(f'{self.month} data downloaded')
    ...
    def write_unzipped_pgn(self):
        ...
        read_file = f'{self.folder}/{self.month}.pgn.bz2'
        write_file = f'{self.folder}/{self.month}.pgn'
        ...
        self.print_log(f'{self.month} file unzipped')
    ...
    def locate_eval_games(self):
        ...
        rows_to_read = []
        eval_lines_list = []
        unzipped_file = f'{self.folder}/{self.month}.pgn'
        eval_lines_file = f'{self.folder}/{self.month}_eval_lines.pgn'
        ...
        # create eval file
        eval_file = f'{self.folder}/{self.month}_eval.pgn'
            ...
            with open(eval_file, "w") as fout:
                for tupl in rows_to_read:
                    for line in itertools.islice(fin, tupl[0], tupl[1]):
                        fout.write(line)
        self.print_log(f'{self.month}_eval_lines.pgn and {self.month}.pgn written')
    ...
    def create_games(self, chunk_size):
        ...
        with open(eval_file, 'r') as f:
            while True:
                file = f'{self.folder}/{chunk}_{games_file}'
                with open(file, "w") as fout:
                    self.games_list.append(file)
                    # write headers
                    fout.write('game_type,game_id,utc_date,utc_time,white,black,result,white_elo,black_elo,termination,month\n')                             
                    ...
                    for j in range(1,chunk_size+1):
                        ...
                        # when elo = '?', set to null
                        if white_elo == '?':
                            white_elo = 'NaN'
                        if black_elo == '?':
                            black_elo = 'NaN'

                        fout.write(f'{game_type},{game_id},{utc_date},{utc_time},{white},{black},{result},{white_elo},{black_elo},{termination},{self.month}\n')
                        games_d[i] = game_id
                        i += 1
        ...
        self.print_log(f'{self.month} games_files written with {i} rows')
    ...
    def create_moves(self, games_chunk_size, moves_chunk_size):
        ...
        with open(eval_lines_file, 'r') as f:
            while True:
                file = f'{self.folder}/{chunk}_{moves_file}'
                with open(file, "w") as fout:
                    self.moves_list.append(file)
                    # write headers
                    fout.write('game_id,move_number,move,eval,white,month\n')
                    for i, line in enumerate(f,1):
                        ...
                        # write to moves table
                            ...
                            for j, move in enumerate(split_line):
                                ...
                                fout.write(f'{games_d[k]},{j+1},{move_value},{eval_value},{white},{self.month}\n') 
                        k +=1
                ...
                if i != moves_chunk_size:
                    break
        self.print_log(f'{self.month} moves_files written')
    ...
    def load_to_postgres(self):
        ...
        for table_list in list_lists:
            with tqdm(total=len(table_list)) as pbar:
                for i, file_name in enumerate(table_list, 1):
                    df = pd.read_csv(file_name)
                    table_name = file_name[-9:-4]
                    df.to_sql(table_name, engine, index=False, if_exists='append')
                    pbar.update(i)
            print(f'{self.month} {table_name} inserted to postgres')

        self.print_log(f'{self.month} data written to postgres')
